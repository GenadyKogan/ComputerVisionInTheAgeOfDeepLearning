...
class Khatt():
    ...
    def get_split_window_for_specific_image(self, image_name):
        ...
        for y in range(0, image.shape[0] - w_height, stepSize):
            for x in range(0, image.shape[1] - w_width, stepSize):
                ...
                cv2.rectangle(tmp, (x, y), (x + w_width,
                              y + w_height), (0, 0, 0))
        return np.array(tmp)
    ...
    def x_preprocessing(self, df, size):
        ...
        for image_name in df['image_path']:
            ...
            # Resize the image to a uniform size (32,32)
            input_img_resize = cv2.resize(tmp, (size, size))
            # Size after reshape is (32, 32, 3)
            input_img_reshape = np.reshape(input_img_resize, (size, size, 3))
            # Append all the data to dataframe
            df_data_image = df_data_image.append(
                {'img_data': input_img_reshape}, ignore_index=True)
        # Convert a dataset images to np.array
    ...
    def get_cnn_model(self, size):
        ...
        model.add(Conv2D(filters=32, kernel_size=(
            3, 3), activation='relu', padding='same', input_shape=(size, size, 3)))
    ...
    def run_transfer_learning(self, preTrained_conv, name_save):
        ...
        model.fit(self.x_train, self.y_train, validation_data=(
            self.x_test, self.y_test), epochs=20, batch_size=256)
    ...
    def run_model_type(self, choice):
        ...
        # Get the size of a specific model
        size = self.model_dict.get(name_model_file)
        self.y_train, self.x_train, self.y_test, self.x_test = self.get_train_and_test(
            choice)
        if choice == 0:
            # ------------ CNN model
            ...
            transfer_learning_type_list = [
                vgg16.VGG16, ResNet50, Xception, EfficientNetB0]
    ...
    def plot_loss_and_accurecy_model(self, history, name, save_nane):
        ...
        plt.subplots_adjust(top=1.00, bottom=0.0, left=0.0,
    ...
    def write_results_to_file(self, df):
        ...
        for row in df.iloc:
            ...
            # Write the data
            # ------------- a
            file.write("Name of the model: " +
                       str(row['name_model_file']) + '\n\n')
            # ------------- b
            model.summary(print_fn=lambda x: file.write(x + '\n'))
            # ------------- c
            file.write("Overall accuracy of the model: " +
                       str(row['acc_score']) + '\n\n')
            # ------------- d
